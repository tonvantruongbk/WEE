    ...
    { 
        ...
        public JsonResult DataHandler(DatatablesRequest request)
        {
            try
            {
                ...
                ReponseToDatatables<JobType> result = new ReponseToDatatables<JobType>
                {
                    draw = request.Draw,
                ...
                if (request.FilterBase != null)
                {
                    foreach (var dtFilterBase in request.FilterBase)
                    {
                        ...
                        try
                        {
                            ...
                            if (field != null)
                            {
                                var bbb = db.JobType.Select("new ("+ dtFilterBase.Ydacf_FieldName + " as label, " + dtFilterBase.Ydacf_FieldName + " as value)");
        ...
        [HttpPost]
        public JsonResult Create(JobType data)
        {
            db.JobType.Add(data);
            db.SaveChanges();
        ...
        [HttpPost]
        public JsonResult Edit(int Id, JobType data)
        {
            if (data != null)
            {
                data.JobTypeID = Id;
        ...
        {
            JobType entity = db.JobType.FirstOrDefault(a => a.JobTypeID == id);
            if (entity != null)
            {
                db.JobType.Remove(entity);
                db.SaveChanges();
            }
        ...
        {
          var result =  db.JobType.Select(a => new CommonModel {label = a.JobTypeName, value = a.JobTypeID}).ToList();
