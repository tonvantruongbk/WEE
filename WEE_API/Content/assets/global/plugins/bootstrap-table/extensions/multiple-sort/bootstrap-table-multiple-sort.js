...
(function($) {
    ...
    BootstrapTable.prototype.initToolbar = function() {
        ...
        if (this.options.showMultSort) {
            var $btnGroup = this.$toolbar.find('>.btn-group').first(),
                $multSortBtn = this.$toolbar.find('div.multi-sort');

            if (!$multSortBtn.length) {
                $multSortBtn = '  <button class="multi-sort btn btn-default' + (this.options.iconSize === undefined ? '' : ' btn-' + this.options.iconSize) + '" type="button" data-toggle="modal" data-target="' + sortModalId + '" title="' + this.options.formatMultipleSort() + '">';
                $multSortBtn += '     <i class="' + this.options.iconsPrefix + ' ' + this.options.icons.sort + '"></i>';
                $multSortBtn += '</button>';

                $btnGroup.append($multSortBtn);
    ...
    BootstrapTable.prototype.addLevel = function(index, sortPriority) {
        ...
        var $multSortName = this.$sortModal.find('.multi-sort-name').last(),
            $multSortOrder = this.$sortModal.find('.multi-sort-order').last();

        $.each(this.columns, function (i, column) {
            if (column.sortable === false || column.visible === false) {
                return true;
            }
            $multSortName.append('<option value="' + column.field + '">' + column.title + '</option>');
        });

        $.each(sort_order, function(value, order) {
            $multSortOrder.append('<option value="' + value + '">' + order + '</option>');
        });

        if (sortPriority !== undefined) {
            $multSortName.find('option[value="' + sortPriority.sortName + '"]').attr("selected", true);
            $multSortOrder.find('option[value="' + sortPriority.sortOrder + '"]').attr("selected", true);
        }
    };
