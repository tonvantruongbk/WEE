...
{ 
    ...
    { 
        ...
        public JsonResult DataHandler(DatatablesRequest request)
        {
            try
            {
                ...
                var all = db.Question
                            ...
                            .AsNoTracking();
                if (!string.IsNullOrEmpty(Session["QuestionTypeID"] + ""))
                {
                    id = Convert.ToInt64(Session["QuestionTypeID"] + "");
                    all = db.Question.Where(a => a.QuestionTypeID == id)
                            .Include(a=>a.QuestionType)
                        .AsNoTracking();
                }
                ...
                ReponseToDatatables<Question> result = new ReponseToDatatables<Question>
                {
                    draw = request.Draw,
                ...
                if (request.FilterBase != null)
                {
                    foreach (var dtFilterBase in request.FilterBase)
                    {
                        ...
                        try
                        {
                            ...
                            if (field != null)
                            {
                                var bbb = db.Question.Select("new ("+ dtFilterBase.Ydacf_FieldName + " as label, " + dtFilterBase.Ydacf_FieldName + " as value)");
        ...
        [HttpPost]
        public JsonResult Create(Question data)
        {
            db.Question.Add(data);
            db.SaveChanges();
        ...
        [HttpPost]
        public JsonResult Edit(int Id, Question data)
        {
            if (data != null)
            {
                data.QuestionID = Id;
        ...
        {
            Question entity = db.Question.FirstOrDefault(a => a.QuestionTypeID == id);
            if (entity != null)
            {
        ...
        {
          var result =  db.Question.Select(a => new CommonModel {label = a.QuestionName, value = a.QuestionID}).ToList();
